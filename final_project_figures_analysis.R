...
figure2
## Q2-4 for v2 
temp2 = aggregate(mydata, by=list(mydata$v2_item,mydata$PVI), mean)
temp2 <- mydata %>%
  group_by(v2_item,PVI) %>% 
  summarise( 
    n=159,
    mean1=mean(v2_Q2),
    mean2=mean(v2_Q3),
  ...
  mutate( ci2=se2 * qt((1-0.05)/2 + .5, n-1))  %>%
  mutate( ci3=se3 * qt((1-0.05)/2 + .5, n-1))
temp2_long = melt(temp2, id.vars = c("v2_item", "PVI"))
temp2_long = cbind(temp2_long[7:24,],temp2_long[61:78,])
temp2_long = temp2_long[,c(1,2,3,4,8)]
colnames(temp2_long) = c("v1_item","PVI","variable","m","ci")

PVI_labs = c("no PVI","PVI")
names(PVI_labs) = c("0","1")

figure3 = ggplot(temp2_long, aes(x = factor(v1_item,levels=c("coin","card","handkerchief")), y = m, group = variable, fill = variable)) + 
